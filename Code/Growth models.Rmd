...
```{r}
plot(FL..cm. ~ AgeClass , data=mydata, col = as.factor(State))
```

Drop unaged 0 increments
```{r}
mydata2 <- mydata %>% filter(AgeClass != 0)
plot(FL..cm. ~ Months , data=mydata2)#, col = as.factor(State))
plot(Capture.Month ~ Months , data=mydata2)#, col = as.factor(State))
mydata2$AgeMonths <- AgeMonths(mydata2$AgeClass, mydata2$Capture.Month, 9)
plot(Capture.Month ~ AgeMonths , data=mydata2)#, col = as.factor(State))
plot(FL..cm. ~ AgeMonths , data=mydata2)#, col = as.factor(State))
mydata2 <- mydata2 %>% mutate(AgeMonths = case_when((AgeClass < 1) ~ AgeClass*12,
                              TRUE ~ AgeMonths))
plot(FL..cm. ~ AgeMonths , data=mydata2)#, col = as.factor(State))
mydata2 <- mydata2 %>% select(-Months)
library(tidyverse)
write_csv(mydata2, "../Data/2021 growth modelling data.csv")
```
